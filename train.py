...
if __name__ == '__main__':
    ...
    train_loader.dataset.set_scale(0)
    val_loaders = loader.loader_test
    ...
    for epoch in range(args.epochs):
        ...
        for lr_imgs, hr_imgs, _ in train_bar:
        ...
        with torch.no_grad():
            for val_loader in val_loaders:
                val_loader.dataset.set_scale(0)
                psnr_values = []
                val_bar = tqdm(
                    val_loader,
                    desc='Validation',
                )
                for lr_imgs, hr_imgs, _ in val_bar:
                    lr_imgs = lr_imgs.to('cuda')
                    hr_imgs = hr_imgs.to('cuda')
                    preds = model(lr_imgs)
                    psnr_values.append(psnr(hr_imgs, preds).item())
                avg_psnr = sum(psnr_values) / len(psnr_values)
                print(f'Epoch: {epoch + 1}/{args.epochs}, {val_loader.dataset.name}, PSNR: {avg_psnr:.2f}')
        model.train()
